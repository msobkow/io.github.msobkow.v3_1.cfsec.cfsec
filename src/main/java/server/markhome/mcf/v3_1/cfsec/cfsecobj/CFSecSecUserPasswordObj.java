...
public class CFSecSecUserPasswordObj
	implements ICFSecSecUserPasswordObj
{
	...
	public ICFSecSecUserPasswordObj realise() {
		ICFSecSecUserPasswordObj retobj = ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().realiseSecUserPassword(
	...
	@Override
	public void forget() {
		((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().reallyDeepDisposeSecUserPassword( (ICFSecSecUserPasswordObj)this );
	}

	@Override
	public ICFSecSecUserPasswordObj read() {
		ICFSecSecUserPasswordObj retobj = ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().readSecUserPasswordByIdIdx( getPKey(), false );
		return( (ICFSecSecUserPasswordObj)retobj );
	}

	@Override
	public ICFSecSecUserPasswordObj read( boolean forceRead ) {
		ICFSecSecUserPasswordObj retobj = ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().readSecUserPasswordByIdIdx( getPKey(), forceRead );
		return( (ICFSecSecUserPasswordObj)retobj );
	}

	@Override
	public ICFSecSecUserPasswordTableObj getSecUserPasswordTable() {
		return( ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj() );
	}
	...
	public ICFSecSecUserPassword getRec() {
		if( rec == null ) {
			if( isNew ) {
				...
				rec = getSchema().getCFSecBackingStore().getTableSecUserPassword().readDerivedByIdIdx( ((ICFSecSchemaObj)getSchema()).getAuthorization(),
						getPKey() );
				if( rec != null ) {
					copyRecToPKey();
				}
	...
	public ICFSecSecUserPasswordEditObj beginEdit() {
		...
		if( getIsNew() ) {
			...
			lockobj = ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().lockSecUserPassword( getPKey() );
		}
		edit = ((ICFSecSchemaObj)getSchema()).getSecUserPasswordTableObj().newEditInstance( lockobj );
