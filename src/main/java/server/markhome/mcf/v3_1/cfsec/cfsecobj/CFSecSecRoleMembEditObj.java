// Description: Java 25 edit object instance implementation for CFSec SecRoleMemb.
...
public class CFSecSecRoleMembEditObj
	implements ICFSecSecRoleMembEditObj
{
	...
	public CFSecSecRoleMembEditObj( ICFSecSecRoleMembObj argOrig ) {
		orig = argOrig;
		getRec();
	...
	public ICFSecSecRoleMembObj realise() {
		...
		ICFSecSecRoleMembObj retobj = getSchema().getSecRoleMembTableObj().realiseSecRoleMemb( (ICFSecSecRoleMembObj)this );
		return( retobj );
	}
	...
	public ICFSecSecRoleMembObj create() {
		copyRecToOrig();
		ICFSecSecRoleMembObj retobj = ((ICFSecSchemaObj)getOrigAsSecRoleMemb().getSchema()).getSecRoleMembTableObj().createSecRoleMemb( getOrigAsSecRoleMemb() );
	...
	public ICFSecSecRoleObj getRequiredContainerRole( boolean forceRead ) {
		if( forceRead || ( requiredContainerRole == null ) ) {
			...
			if( ! anyMissing ) {
				ICFSecSecRoleObj obj = ((ICFSecSchemaObj)getOrigAsSecRoleMemb().getSchema()).getSecRoleTableObj().readSecRoleByIdIdx( getPKey().getRequiredSecRoleId() );
	...
	public ICFSecSecUserObj getRequiredParentUser( boolean forceRead ) {
		if( forceRead || ( requiredParentUser == null ) ) {
			boolean anyMissing = false;
			if( ! anyMissing ) {
				ICFSecSecUserObj obj = ((ICFSecSchemaObj)getOrigAsSecRoleMemb().getSchema()).getSecUserTableObj().readSecUserByULoginIdx( getPKey().getRequiredLoginId() );
