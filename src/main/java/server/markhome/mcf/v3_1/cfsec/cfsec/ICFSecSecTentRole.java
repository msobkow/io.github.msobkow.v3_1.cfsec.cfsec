// Description: Java 25 interface for a SecTentRole record implementation
...
/**
 *	ICFSecSecTentRole persistence instances have CodeVis Protected, meaning that any only user interfaces and application code specific to the defining schema can access it.
 */
public interface ICFSecSecTentRole
{
	...
	public default void setRequiredContainerSysRole(ICFSecProtSecSysGrp argObj) {
		if (argObj == null) {
			setRequiredContainerSysRole((String)null);
		}
		else {
	...
	public default void setRequiredContainerSysRole(ICFSecPubSecSysGrp argObj) {
		if (argObj == null) {
			setRequiredContainerSysRole((String)null);
		}
		else {
