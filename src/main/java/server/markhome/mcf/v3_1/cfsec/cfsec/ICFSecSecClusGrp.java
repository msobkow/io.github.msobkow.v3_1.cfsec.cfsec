// Description: Java 25 interface for a SecClusGrp record implementation
...
/**
 *	ICFSecSecClusGrp persistence instances have CodeVis Protected, meaning that any only user interfaces and application code specific to the defining schema can access it.
 */
public interface ICFSecSecClusGrp
{
	...
	public default void setRequiredOwnerCluster(ICFSecProtCluster argObj) {
		if (argObj == null) {
			setRequiredOwnerCluster((CFLibDbKeyHash256)null);
		}
		else {
	...
	public default void setRequiredOwnerCluster(ICFSecPubCluster argObj) {
		if (argObj == null) {
			setRequiredOwnerCluster((CFLibDbKeyHash256)null);
		}
		else {
